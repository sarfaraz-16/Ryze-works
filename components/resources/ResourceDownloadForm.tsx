...
export function ResourceDownloadForm({ resourceTitle, resourceSlug, fileSize }: Props) {
    ...
    try {
      const res = await fetch("/api/leads", {
        ...
        body: JSON.stringify({
          ...
          company,
          type: "general",
          service_interest: `Toolkit: ${resourceTitle}`,
          message: `Requested toolkit: ${resourceTitle} (${resourceSlug})`,
          source: "resources-gated-download",
        }),
      });
    ...
    return (
      <div className="rounded-3xl bg-[#0d0e17]/95 border border-emerald-500/30 p-8 text-center space-y-5 shadow-2xl shadow-emerald-500/10 backdrop-blur-xl">
        ...
        <a
          ...
          onClick={(e) => {
            e.preventDefault();
            alert(`Initiating verified download for: ${resourceTitle} (${fileSize})`);
          }}
        ...
        >
          ...
          <span>Download Toolkit ({fileSize})</span>
        </a>
      </div>
    );
  }
  ...
  return (
    <div className="sticky top-28 bg-[#0B0813]/80 backdrop-blur-2xl border border-white/10 hover:border-violet-500/30 rounded-3xl p-6 sm:p-8 shadow-[0_25px_60px_rgba(0,0,0,0.7)] relative overflow-hidden space-y-6">
      ...
      <div className="flex items-center gap-3 pb-4 border-b border-white/10 relative z-10">
        ...
        <div>
          <h3 className="text-sm font-bold text-white">Instant Toolkit Download</h3>
          <p className="text-[11px] text-zinc-400">Complete verification to unlock asset files.</p>
        </div>
      </div>
